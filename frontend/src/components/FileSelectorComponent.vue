...
import { base64ToFile, bitTomb, mbToBit } from 'src/lib/helpers';

export default {
    components: { FileImageComponent },
    props: ['value', 'formField', 'field', 'errorMessage', 'scanable', 'disabled'],
    methods: {
        ...
        handleRemove(file) {
            this.files = this.files.filter((fileObj) => fileObj?.name !== file?.name);
        },
        verifyExt(file) {
