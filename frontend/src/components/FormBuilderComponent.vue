<template>
    <div>
        ...
        >
                    ...
                    <div :class="field?.className">
                        ...
                        >
                            <template v-slot:prepend>
                                <q-icon v-if="field?.prepend_icon" :name="field?.prepend_icon" />
                            </template>
                            <template v-slot:append>
                                <span class="text-xs">{{ field?.append_text }}</span>
                            </template>
                        </q-input>
                        ...
                        <div v-else-if="field?.type === 'server-select'">
                            <new-select-component
                                ...
                                :server-search-able="field?.server_search_able ?? false"
                                ...
                                :on-change="
                                    (data) => {
                                        handleSelectChange(field, data, formField);
                        ...
                        <div v-else-if="field.type === 'select'">
                            <q-select
                                ...
                                :error="!!errorMessage"
                                outlined
                                @update:model-value="(value) => formField.onChange(value)"
                            />
                        </div>
                        <div v-else-if="field.type === 'radio'">
                            ...
                            <q-radio
                                ...
                                @update:model-value="
                                    (radioValue) => {
                                        if (typeof field?.on_change !== 'undefined') {
                        ...
                        <div v-else-if="field.type === 'money'">
                            <money-input
                                ...
                                :on-change="(moneyValue) => formField.onChange(moneyValue)"
                            />
                        </div>
                        <div
                            v-if="field.type === 'radio_buttons'"
                            :class="!!errorMessage && 'border border-red-500 p-3 text-red-500 my-2'"
                        >
                            ...
                            <q-radio
                                ...
                                :val="option"
                                @update:model-value="(radioValue) => formField.onChange(radioValue)"
                            />
                            <p class="text-xs text-red-500 mb-3" v-if="!!errorMessage">
            ...
            <form-values-listener :form-values="values" :on-values-changes="onValuesChanged" />
        </validation-form>
    </div>
</template>
...
export default {
    ...
    setup(props) {
        ...
        const resolveUrl = (field, values) => {
            return typeof field.url === 'function' ? field.url(values) : field.url;
        };
    ...
    methods: {
        async submitForm(values, actions) {
            ...
            this.loading = true;
            const checkFileExistence = this.fields.findIndex((field) => field.type === 'file');
            let formValues = null;
            if (checkFileExistence !== -1) {
                ...
                if (typeof this.hiddenFields !== 'undefined') {
                    for (let key in this.hiddenFields) {
                        values[this.hiddenFields[key]['name']] = this.hiddenFields[key]['value'];
                    }
                }
            }
            try {
        ...
        async onSubmit(values, actions) {
            if (this.needConfirmation) {
                this.$q
                    .dialog({
                        title: this.confirmationTitle ?? this.$translate('Confirmation'),
                        message: this.confirmationMessage ?? this.$translate('Are you sure?'),
