<template>
    <div class="md:px-2 md:px-1 py-4" ref="datatablePrintRef" id="datatablePrintRef">
        <q-table
            flat
            bordered
            ref="tableRef"
        ...
        >
            ...
            <template v-slot:body="props">
                ...
                >
                    <q-td class="print:hidden">
                        <q-checkbox dense v-model="props.selected" class="print:hidden" />
                    </q-td>
                    <q-td
                        :props="props"
                    ...
                    >
                        <div
                            @click="typeof col.onClick !== 'undefined' && col?.onClick(props.row)"
                            :class="`${col?.class} ${
                                typeof col?.onClick !== 'undefined' && 'cursor-pointer'
                            }`"
                            v-if="col?.is_image"
                        >
                            <q-img :src="props.row[col.name]" />
                        </div>
                        <div v-else-if="typeof col?.file_viewer !== 'undefined'">
                            <q-btn :label="props.row[col.label]" flat size="sm" color="primary">
                                <q-popup-proxy>
                                    <q-card class="min-w-[400px]">
                                        <q-card-section>
                                            <file-viewer :url="props.row[col.name]" />
                                        </q-card-section>
                                    </q-card>
                                </q-popup-proxy>
                            </q-btn>
                        ...
                        <span
                            ...
                            :class="`${col?.class} ${
                                typeof col?.link !== 'undefined'
                                    ...
                                    : ''
                            } ${conditionalClass(col?.conditionalClass, props.row)} ${
                                typeof col?.cellColor !== 'undefined' && col?.cellColor(props.row)
                            }`"
                        >
                            <router-link
            ...
            <template v-slot:top>
                ...
                >
                    ...
                    <div class="flex items-center w-full md:w-fit flex-row-reverse justify-end">
                                            ...
                                            <q-card-section>
                                                <div class="column">
                                                    ...
                                                    <q-toggle
                                                        ...
                                                        :key="col.name"
                                                        :label="col.label"
                                                        @click="filterTableColumns(col.name)"
                                                    />
                                                </div>
                                            </q-card-section>
                                        </q-card>
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            >
                                <q-btn
                                    v-if="action?.show_on_select ? selectedRow.length > 0 : true"
...
export default {
    components: { FileViewer, ProtectedComponent },
    props: {
        url: {
            type: String,
            default: '',
    ...
    setup(setupProps) {
        ...
        const fetcher = async (filter, page, rowsPerPage, sortBy, descending, router) => {
            loading.value = true;
            try {
                const query = {
                ...
                return await api()
                    .get(`${setupProps.url}?${new URLSearchParams(query).toString()}`)
                    .then((res) => res.data)
        ...
        const onRequest = async (props) => {
            ...
            loading.value = true;
            const data = await fetcher(filter, page, rowsPerPage, sortBy, descending, router);
        ...
        const exportTable = () => {
            const content = [
                options.value.map((col) =>
                    visibleColumns.value.includes(col.name) ? wrapCsvValue(col.label) : null
                ),
            ]
                .concat(
                    rows.value.map((row) =>
            ...
            const status = exportFile(`${setupProps.tableName}.csv`, content, 'text/csv');
        ...
        onMounted(() => {
            ...
            const query = {
                ...
                descending: descending ? descending : pagination.value.descending,
                rowsPerPage: rowsPerPage ? rowsPerPage : pagination.value.rowsPerPage,
            ...
            if (rowsPerPage) pagination.value.rowsPerPage = parseInt(rowsPerPage);
        ...
        const filterTableColumns = (colName) => {
            if (visibleColumns.value.includes(colName)) {
                visibleColumns.value = visibleColumns.value.filter((item) => item !== colName);
            } else {
