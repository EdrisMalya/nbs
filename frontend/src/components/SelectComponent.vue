<template>
    ...
    <div v-else>
        ...
        >
            <template v-if="createdAble && languageStore.direction == 'rtl'" #prepend>
                <q-btn size="xs" icon="add" round color="primary" @click="handleAddClick(true)" />
            </template>
            <template v-else-if="createdAble && languageStore.direction == 'ltr'" #append>
                <q-btn size="xs" icon="add" round color="primary" @click="handleAddClick(true)" />
            </template>
...
export default defineComponent({
    ...
    setup(prop) {
        ...
        watch(created_id, (newValue) => {
            if (newValue !== 0) {
                const index = data?.value?.data.findIndex((obj) => obj.id === newValue);
                if (index !== -1) {
            ...
            if (val === '') {
                ...
                const filteredOptions =
                    this.data.data?.filter((item) =>
                        resolve(this.optionLabel, item).toLowerCase().includes(newValue)
                    ) || [];
                update(
                    () => {
    ...
    watch: {
        ...
        serverData(newValue) {
            if (this.findFromId) {
                const index = newValue?.data.findIndex((obj) => obj.id === this.defaultValue);
                if (index !== -1) {
