                ...
                <q-card-section v-if="!full_page_loading">
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-x-2">
                        ...
                        <div class="col-span-2">
                            ...
                            >
                                <select-component
                                    :url="appRoutes.user_management.roles.api + '?fetch_all'"
                                    :multiple="true"
                                    :default-value="server_user && server_user.roles"
                                    :on-change="
                                        (data) => {
                                            field.onChange(data.length < 1 ? [] : data);
                                        }
                                    "
                                    :error-message="$translate(errorMessage)"
...
export default defineComponent({
    ...
    setup(props) {
        const schema = yup.object({
            ...
            is_active: yup.boolean().required('This section is required').default(true),
    ...
    methods: {
        async onSubmit(values, actions) {
            ...
            try {
                const url = this.user
                    ? appRoutes.user_management.users.api + '/' + this.user.id + '?_method=PUT'
