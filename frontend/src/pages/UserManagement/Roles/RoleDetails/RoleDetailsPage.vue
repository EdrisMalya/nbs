<template>
    <protected-component permission-key="roles-view-details" is-page>
        <div>
            <div class="py-4">
                <p class="text-lg font-semibold">{{ $route.params.name }} role details</p>
                <div v-if="!data" class="flex justify-center py-12">
                    <q-spinner size="3rem" color="primary" />
                </div>
                <div v-else class="py-1">
                    <h3 class="font-semibold">
                        {{ $translate('All assigned permissions to this role') }}
                    </h3>
                    ...
                    <ValidationForm :validation-schema="schema" @submit="handleRoleDetailsSave">
                        <Field
                            name="name"
                        ...
                        <role-permission-groups
                            :data="data"
                            :handle-permission-clicked="handlePermissionClicked"
...
export default {
    ...
    setup() {
        ...
        const { data, mutate } = useSWRV(
            appRoutes.user_management.roles.role_details.api.replace('{id}', route.params.id),
            fetcher
        );
    ...
    methods: {
        handlePermissionClicked(id) {
            if (this.role_permissions.includes(id)) {
                this.role_permissions = this.role_permissions.filter((obj) => obj !== id);
            } else {
    ...
    watch: {
        data(newValue) {
            if (newValue) {
                this.role_permissions = newValue.role.role_permissions?.map((id) => id);
            }
        },
    },
};
