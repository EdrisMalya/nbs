    ...
    <q-dialog :model-value="permissionModel" persistent>
        <q-card class="w-[300px]">
            ...
            <ValidationForm :validation-schema="schema" @submit="onSubmit">
                <q-card-section>
                    <Field name="name" v-slot="{ errorMessage, value, field }" :value="name">
                        <q-input
                            label="Name"
                            outlined
                            dense
...
export default defineComponent({
    ...
    props: ['permissionModel', 'permissionGroupId', 'handleModelClose', 'mutate'],
    setup() {
    ...
    methods: {
        async onSubmit(values, actions) {
            this.loading = true;
            try {
                const result = await api().post(appRoutes.user_management.permission.api, {
                    name: values?.name,
