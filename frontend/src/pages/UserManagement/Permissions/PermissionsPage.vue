<template>
    <div class="p-4">
        <div class="flex justify-between mb-4 items-center mt-5">
            <p class="text-lg font-semibold px-4">List of all permission groups</p>
            <q-btn
                @click="handlePermissionGroupModelOpen(0, 'permission-group', null)"
                color="primary"
                icon-right="add"
                size="small"
            >
        ...
        <div v-if="!data && !error" class="p-12 flex items-center justify-center">
            <q-spinner size="2rem" color="primary" />
        </div>
        <p v-else-if="data.length < 1" class="text-red-600 text-center py-12">
            No permission group found
        </p>
        <PermissionGroup
            :handle-permission-group-model-open="handlePermissionGroupModelOpen"
            v-else
            :mutate="mutate"
            :data="data"
...
export default defineComponent({
    ...
    methods: {
        ...
        applyDrag(arr, dragResult) {
            ...
            api().post(
                appRoutes.user_management.permission.permission_group_api + '/update-order',
                {
                    parent_id: 0,
                    removedIndex,
                    addedIndex,
