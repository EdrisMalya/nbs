                        ...
                        <q-card-section>
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <field
                                        name="customer_id"
                                        :value="$generalStore?.store_settings?.customer?.id"
                                    ...
                                    >
                                        <select-component
                                            ...
                                            icon="person"
                                            :url="
                                                appRoutes.user_management.customers.api +
                                                '?for-api=true'
                                            "
                                            ...
                                            :default-value="{
                                                label:
                                                    $generalStore.store_settings.customer
                                                        .first_name +
                                                    ' ' +
                                                    $generalStore.store_settings.customer.last_name,
                                                value: $generalStore.store_settings.customer.id,
                                            }"
                                            :on-change="
                                                (value) => {
                                ...
                                <div>
                                    <field
                                        name="warehouse_id"
                                        :value="$generalStore?.store_settings?.warehouse?.id"
                                        v-slot="{ errorMessage, field }"
                                    >
                                        <select-component
                                            icon="home"
                                            :url="
                                                appRoutes.configurations.systemConfiguration
                                                    .warehouse.api + '?for-api=true'
                                            "
                                            :error-message="$translate(errorMessage)"
                                            option-label="name"
                                            option-value="id"
                                            :default-value="{
                                                label: $generalStore.store_settings.warehouse.name,
                                                value: $generalStore.store_settings.warehouse.id,
                                            }"
                                            :on-change="
                                                (value) => {
                                                    ...
                                                    posStore.setWarehouse(value?.value);
                                                }
                                            "
                        ...
                        <q-card-section>
                            ...
                            >
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 fixed w-full bottom-0 bg-white">
                                        <div class="space-y-2">
                                            <q-input
                                                dense
                                                v-model="pmStore.tax"
                                        ...
                                        <div class="text-right mt-12">
                                            ...
                                            <p class="text-xl font-bold">
                                                {{ $translate('Total') }}:
                                                {{
                                                    parseFloat(pmStore.subtotal()).toLocaleString()
                                                }}
                                                {{ pmStore?.currency?.symbol }}
                                            </p>
                                        </div>
                                        <div class="col-span-2 grid grid-cols-3 gap-2 mt-3">
                                            <q-btn
                        ...
                        <q-card-section>
                            <div class="flex items-center space-x-3">
                                <div class="flex-1 -mt-5">
                                    <product-select
                                        :on-change="(product) => handleAddProduct(product)"
                                        :disable-label="true"
                                ...
                                <div>
                                    ...
                                    >
                                        <select-component
                                            ...
                                            :url="
                                                appRoutes.configurations.systemConfiguration
                                                    .currencyConfiguration.api + '?api=true'
                                            "
                                            :on-change="
                                                (data) => {
                                ...
                                <div class="-mt-5 space-x-1">
                                    ...
                                    <q-btn
                                        ...
                                        color="primary"
                                    >
                                        <q-tooltip>{{ $translate('Exhcnage product') }}</q-tooltip>
                                    </q-btn>
                                    <q-btn
                                        ...
                                        color="primary"
                                    >
                                        <q-tooltip>{{ $translate('Direct sale') }}</q-tooltip>
                                    </q-btn>
                                    <q-btn icon="dashboard" outline color="primary" to="/" />
...
export default {
    ...
    methods: {
        ...
        async handlePrintInvoice(id) {
            this.$q.loading.show();
            try {
                const result = await api().get(appRoutes.direct_sale.api + '/' + id);
                this.direct_sale_invoice = {
