...
export default defineComponent({
    ...
    methods: {
        async runBackup() {
            this.loading = true;
            try {
                const result = await api().get(this.appRoutes.configurations.backup.api);
        ...
        restoreBackup(path) {
            this.$q
                .dialog({
                    ...
                    message: this.$translate('Are you sure you want to restore'),
                    cancel: true,
                })
                .onOk(async () => {
                    this.$q.loading.show({
                    ...
                    try {
                        const result = await api().post(appRoutes.configurations.backup.index_api, {
                            path: path,
                        });
                        if (!result.data.result) {
                            this.$q.notify({
